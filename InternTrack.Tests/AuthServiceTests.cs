    ...
    userRepositoryMock.Verify(
        repository => repository.AddAsync(
            It.Is<User>(user =>
                ...
                user.Intern != null &&
                user.Intern.Name == dto.Name &&
                user.Intern.Surname == dto.Surname &&
                user.Intern.Email == dto.Email &&
                user.Intern.DepartmentId == departmentId)),
        Times.Once);
}

[Fact]
public async Task ChangePasswordAsync_UserDoesNotExist_ShouldReturnNotFound()
{
    // ARRANGE
    var userRepositoryMock = new Mock<IUserRepository>();
    var departmentRepositoryMock = new Mock<IDepartmentRepository>();
...
public async Task RefreshAsync_UserDoesNotExist_ShouldReturnValidationError()
{
    ...
    userRepositoryMock
        .Setup(repository => repository.GetByIdAsync(storedRefreshToken.UserId))
        .ReturnsAsync((User?)null);
...
public async Task LoginAsync_InternWithoutInternProfile_ShouldReturnValidationError()
{
    ...
    tokenServiceMock.Verify(
        service => service.CreateAccessToken(It.IsAny<User>()),
        Times.Never);

    tokenServiceMock.Verify(
        service => service.CreateRefreshToken(),
        Times.Never);
...
public async Task RefreshAsync_InternWithoutInternProfile_ShouldRevokeTokensAndReturnValidationError()
{
    ...
    refreshTokenRepositoryMock
        .Setup(repository =>
            repository.GetByTokenAsync("valid-refresh-token"))
    ...
    // ACT
    var result =
        await service.RefreshAsync("valid-refresh-token");
    ...
    tokenServiceMock.Verify(
        service =>
            service.CreateAccessToken(It.IsAny<User>()),
        Times.Never);
    ...
    userRepositoryMock.Verify(
        repository => repository.AddAsync(
            It.Is<User>(user =>
                ...
                user.Intern != null &&
                user.Intern.Name == "Eda" &&
                user.Intern.Surname == "Şahin" &&
...
public async Task UpdateProfileAsync_EmailCaseOnlyChange_ShouldSkipDuplicateCheck()
{
    ...
    userRepositoryMock.Verify(
        repository =>
            repository.EmailExistsAsync(It.IsAny<string>()),
        Times.Never);
