...
public class InternServiceTests
{
    ...
    public async Task ReactivateInternAsync_InternDoesNotExist_ShouldReturnNotFound()
    {
        ...
        var userRepositoryMock = new Mock<IUserRepository>();
        var internId = 99;
        internRepositoryMock.Setup(repository => repository.GetByIdIncludingInactiveAsync(internId)).ReturnsAsync((Intern? )null);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
    ...
    public async Task ReactivateInternAsync_InternAlreadyActive_ShouldReturnConflict()
    {
        ...
        internRepositoryMock.Setup(repository => repository.GetByIdIncludingInactiveAsync(internId)).ReturnsAsync(intern);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
    ...
    public async Task UpdateAsync_DepartmentDoesNotExist_ShouldReturnValidationError()
    {
        ...
        // ACT
        var result = await service.UpdateAsync(internId, dto);

        // ASSERT
        Assert.False(result.Success);
        internRepositoryMock.Verify(repository => repository.UpdateAsync(It.IsAny<Intern>()), Times.Never);
    }
    ...
    public async Task UpdateAsync_InternDoesNotExist_ShouldReturnNotFound()
    {
        ...
        Assert.Equal(ResultType.NotFound, result.Type);
        internRepositoryMock.Verify(repository => repository.UpdateAsync(It.IsAny<Intern>()), Times.Never);
        departmentRepositoryMock.Verify(repository => repository.GetByIdAsync(It.IsAny<int>()), Times.Never);
    }
    ...
    public async Task UpdateAsync_ValidIntern_ShouldUpdateSuccessfully()
    {
        ...
        internRepositoryMock.Setup(repository => repository.GetByIdAsync(internId)).ReturnsAsync(existingIntern);
        departmentRepositoryMock.Setup(repository => repository.GetByIdAsync(departmentId)).ReturnsAsync(department);
        ...
        internRepositoryMock.Setup(repository => repository.EmailExistsAsync(dto.Email)).ReturnsAsync(false);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
        ...
        internRepositoryMock.Verify(repository => repository.UpdateAsync(existingIntern), Times.Once);
    ...
    public async Task UpdateAsync_EmailAlreadyUsedByAnotherIntern_ShouldReturnConflict()
    {
        ...
        userRepositoryMock.Setup(repository => repository.GetByIdAsync(userId)).ReturnsAsync(user);
        userRepositoryMock.Setup(repository => repository.EmailExistsAsync(dto.Email)).ReturnsAsync(false);
        internRepositoryMock.Setup(repository => repository.EmailExistsAsync(dto.Email)).ReturnsAsync(true);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
        ...
        Assert.False(result.Success);
        Assert.Equal(ResultType.Conflict, result.Type);
        Assert.Equal("Bu email adresi başka bir stajyer tarafından kullanılıyor.", result.Message);
        internRepositoryMock.Verify(repository => repository.UpdateAsync(It.IsAny<Intern>()), Times.Never);
        userRepositoryMock.Verify(repository => repository.UpdateAsync(It.IsAny<User>()), Times.Never);
    }
    ...
    public async Task UpdateAsync_LinkedUserDoesNotExist_ShouldReturnNotFound()
    {
        ...
        Assert.False(result.Success);
        Assert.Equal(ResultType.NotFound, result.Type);
        Assert.Equal("Stajyere bağlı kullanıcı hesabı bulunamadı.", result.Message);
        internRepositoryMock.Verify(repository => repository.UpdateAsync(It.IsAny<Intern>()), Times.Never);
        userRepositoryMock.Verify(repository => repository.UpdateAsync(It.IsAny<User>()), Times.Never);
    }
    ...
    public async Task UpdateAsync_EmailUnchanged_ShouldSkipDuplicateEmailChecksAndUpdateSuccessfully()
    {
        ...
        internRepositoryMock.Verify(repository => repository.EmailExistsAsync(It.IsAny<string>()), Times.Never);
        internRepositoryMock.Verify(repository => repository.UpdateAsync(existingIntern), Times.Once);
        userRepositoryMock.Verify(repository => repository.UpdateAsync(user), Times.Once);
    }
    ...
    public async Task UpdateAsync_ValuesWithWhitespace_ShouldTrimAndUpdateSuccessfully()
    {
        ...
        internRepositoryMock.Setup(repository => repository.EmailExistsAsync("updated@example.com")).ReturnsAsync(false);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
    ...
    public async Task GetByIdAsync_InternDoesNotExist_ShouldReturnNotFound()
    {
        ...
        // ACT
        var result = await service.GetByIdAsync(internId, userId, Roles.Intern);
    ...
    public async Task GetByIdAsync_InternTriesToAccessAnotherIntern_ShouldReturnForbidden()
    {
        ...
        internRepositoryMock.Setup(repository => repository.GetByIdAsync(requestedInternId)).ReturnsAsync(requestedIntern);
        internRepositoryMock.Setup(repository => repository.GetByUserIdAsync(currentUserId)).ReturnsAsync(currentIntern);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
        ...
        // ACT
        var result = await service.GetByIdAsync(requestedInternId, currentUserId, Roles.Intern);

        // ASSERT
        Assert.False(result.Success);
        Assert.Equal(ResultType.Forbidden, result.Type);
        Assert.Equal("Bu stajyer profiline erişim yetkiniz yok.", result.Message);
    }
    ...
    public async Task GetByIdAsync_CurrentInternProfileDoesNotExist_ShouldReturnNotFound()
    {
        ...
        internRepositoryMock.Setup(repository => repository.GetByIdAsync(requestedInternId)).ReturnsAsync(requestedIntern);
        internRepositoryMock.Setup(repository => repository.GetByUserIdAsync(currentUserId)).ReturnsAsync((Intern? )null);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
    ...
    public async Task GetByIdAsync_Admin_ShouldReturnInternWithoutCurrentInternCheck()
    {
        ...
        // ACT
        var result = await service.GetByIdAsync(internId, adminUserId, Roles.Admin);
    ...
    public async Task GetAllAsync_Admin_ShouldReturnAllInterns()
    {
        ...
        // ACT
        var result = await service.GetAllAsync(100, Roles.Admin);
    ...
    public async Task GetAllIncludingInactiveAsync_ShouldReturnActiveAndInactiveInterns()
    {
        ...
        internRepositoryMock.Setup(repository => repository.GetAllIncludingInactiveAsync()).ReturnsAsync(interns);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
        ...
        Assert.NotNull(result.Data);
        Assert.Equal(2, result.Data.Count);
        Assert.Contains(result.Data, intern => intern.Id == 1 && intern.IsActive);
        Assert.Contains(result.Data, intern => intern.Id == 2 && !intern.IsActive);
        internRepositoryMock.Verify(repository => repository.GetAllIncludingInactiveAsync(), Times.Once);
    }
    ...
    public async Task CreateInternWithAccountAsync_DepartmentDoesNotExist_ShouldNotCheckEmailsOrCreateUser()
    {
        ...
        departmentRepositoryMock.Setup(repository => repository.GetByIdAsync(dto.DepartmentId)).ReturnsAsync((Department? )null);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
    ...
    public async Task CreateInternWithAccountAsync_ValidData_ShouldCreateLinkedUserAndInternWithNormalizedValues()
    {
        ...
        departmentRepositoryMock.Setup(repository => repository.GetByIdAsync(dto.DepartmentId)).ReturnsAsync(department);
        userRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(false);
        internRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(false);
    ...
    public async Task CreateInternWithAccountAsync_UserEmailAlreadyExists_ShouldReturnConflictAndSkipInternEmailCheck()
    {
        ...
        departmentRepositoryMock.Setup(repository => repository.GetByIdAsync(dto.DepartmentId)).ReturnsAsync(department);
        userRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(true);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
    ...
    public async Task CreateInternWithAccountAsync_InternEmailAlreadyExists_ShouldReturnConflictAndNotCreateUser()
    {
        ...
        departmentRepositoryMock.Setup(repository => repository.GetByIdAsync(dto.DepartmentId)).ReturnsAsync(department);
        userRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(false);
        internRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(true);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
        ...
        Assert.False(result.Success);
        Assert.Equal(ResultType.Conflict, result.Type);
        Assert.Equal("Bu email adresine ait stajyer kaydı zaten mevcut.", result.Message);
        userRepositoryMock.Verify(repository => repository.AddAsync(It.IsAny<User>()), Times.Never);
    }
    ...
    public async Task CreateInternWithAccountAsync_EmailWithWhitespace_ShouldCheckDuplicatesUsingTrimmedEmail()
    {
        ...
        userRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(false);
        internRepositoryMock.Setup(repository => repository.EmailExistsAsync("eda@example.com")).ReturnsAsync(false);
        userRepositoryMock.Setup(repository => repository.AddAsync(It.IsAny<User>())).Returns(Task.CompletedTask);
        var service = new InternService(
            internRepositoryMock.Object,
            departmentRepositoryMock.Object,
            userRepositoryMock.Object,
        ...
        internRepositoryMock.Verify(repository => repository.EmailExistsAsync("eda@example.com"), Times.Once);
        userRepositoryMock.Verify(
            repository => repository.AddAsync(It.Is<User>(user => user.Email == "eda@example.com")),
            Times.Once);
    }
    ...
    public async Task GetAllAsync_HR_ShouldReturnAllInterns()
    {
        ...
        // ACT
        var result = await service.GetAllAsync(200, Roles.HR);
    ...
    public async Task GetByIdAsync_HR_ShouldReturnInternWithoutCurrentInternCheck()
    {
        ...
        // ACT
        var result = await service.GetByIdAsync(internId, hrUserId, Roles.HR);
