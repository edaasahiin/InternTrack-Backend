...
public class TaskController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Add([FromBody] CreateTaskDto dto)
    {
        if (!CurrentUserHelper.TryGetUserInfo(User, out var userId, out var role))
        {
            return Unauthorized(new { message = "Kullanıcı bilgileri doğrulanamadı." });
        ...
        var result = await _service.AddAsync(dto, userId, role);
    ...
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] UpdateTaskDto dto)
    {
        ...
        var result = await _service.UpdateAsync(id, dto, userId, role);
